...
function Publish-AspNetDocker {
    ...
    process {
		...
		'==== Publish-AspNetDocker ====' | Write-Verbose
        'publishProperties: {0}' -f $publishProperties | Write-Verbose
        'packOutput: {0}' -f $packOutput | Write-Verbose
        'pubxmlFile: {0}' -f $pubxmlFile | Write-Verbose
